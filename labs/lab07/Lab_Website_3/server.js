...
app.get('/player_info/player', function(req,res) {
	...
	var query3 = 'SELECT COUNT(*) AS count FROM football_games WHERE ' + player_id + '=ANY(players);';
	db.task('get-everything', task => {
		return task.batch([
			task.any(query1),
			task.any(query2),
...
app.post('/player_info/add_player', function(req, res){
	...
	var yards_pa = req.body.player_passing_yards;
	var yards_ru = req.body.player_rushing_yards;
	var yards_re = req.body.player_receiving_yards;
	var postit = "INSERT INTO football_players(name,year,major,passing_yards,rushing_yards,receiving_yards) VALUES('" + name + "','" + year + "','" + major + "'," + yards_pa + "," + yards_ru + "," + yards_re + ");"
...
app.post('/player_info/add_game', function(req, res){
	var name_vis = req.body.visitor_name;
	var score_home = req.body.home_score;
	var score_opp = req.body.visitor_score;
	var g_date = req.body.game_date;
	var plyr = req.body.players;
	var postit = "INSERT INTO football_games(visitor_name,home_score,visitor_score,game_date,players) VALUES('" + name_vis + "'," + score_home + "," + score_opp + ",'" + g_date + "','" + plyr + "');";
